...
async function GET() {
    try {
        ...
        const transactions = await db.collection(COLLECTION_NAME).find({}).sort({
            createdAt: -1
        }).toArray();
        // Get all unique dealer IDs
        const dealerIds = [
            ...new Set(transactions.map((t)=>t.dealerId).filter((id)=>id))
        ];
        const dealers = dealerIds.length > 0 ? await db.collection('dealers').find({
            _id: {
                $in: dealerIds.map((id)=>new __TURBOPACK__imported__module__$5b$externals$5d2f$mongodb__$5b$external$5d$__$28$mongodb$2c$__cjs$2c$__$5b$project$5d2f$node_modules$2f$mongodb$29$__["ObjectId"](id))
            }
        }).toArray() : [];
        // Create dealer name map
        const dealerMap = new Map(dealers.map((d)=>[
                d._id.toString(),
                d.name
            ]));
        ...
        if (body.dealerId) {
            try {
                const dealer = await db.collection('dealers').findOne({
                    _id: new __TURBOPACK__imported__module__$5b$externals$5d2f$mongodb__$5b$external$5d$__$28$mongodb$2c$__cjs$2c$__$5b$project$5d2f$node_modules$2f$mongodb$29$__["ObjectId"](body.dealerId)
                });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0a6deee6._.js.map